...
import PreprocessedVideoPlayer from "@/components/PreprocessedVideoPlayer";
      ...
      <div className="w-full max-w-3xl mx-auto space-y-8 relative z-10">
        <div className="text-center space-y-4">
          ...
          <h1 className="text-5xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-blue-400">
            Preprocessed Pitch Transposer
          </h1>
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            Higher quality pitch shifting with advanced processing for the best
            audio experience.
          </p>

          {!videoData && (
            <div className="flex flex-wrap gap-2 justify-center mt-2">
              <span className="px-3 py-1 bg-white/10 rounded-full text-sm text-purple-300 flex items-center">
                <Music className="h-3 w-3 mr-1" /> Superior audio quality
              </span>
              <span className="px-3 py-1 bg-white/10 rounded-full text-sm text-purple-300 flex items-center">
                <Wand2 className="h-3 w-3 mr-1" /> Advanced algorithms
              </span>
            </div>
          )}
        </div>
        ...
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-12 bg-white/5 backdrop-blur-sm rounded-xl p-8 border border-white/10">
            <div className="relative">
              ...
              <div className="absolute inset-0 h-16 w-16 rounded-full border-t-2 border-purple-400 animate-ping opacity-20"></div>
            </div>
            <h3 className="text-xl font-medium mb-2">
              Processing your video...
            </h3>
            <p className="text-gray-400 text-center">
              Creating a high-quality version with optimized pitch shifting.
        ...
        {videoData && !isLoading && (
          <div className="space-y-4">
            <PreprocessedVideoPlayer videoData={videoData} />
          </div>
        )}
      </div>
    </main>
